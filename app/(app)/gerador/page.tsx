...
const perfilOptions = perfisCliente.map((p) => ({
  value: p.value,
  label: p.label,
}));
  ...
  async function melhorar(variante: Variante) {
    ...
    try {
      ...
      if (res.ok && data.texto) {
        setRespostas((prev) =>
          prev ? { ...prev, [variante]: data.texto } : prev,
        );
        setSalvo(false);
      }
    } catch {
        ...
        <div className="rounded-2xl border border-navy-500 bg-navy-700 shadow-card">
          ...
          <div className="space-y-4 p-5 sm:p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <Select
                  id="perfil"
                  value={perfilCliente}
                  onChange={setPerfilCliente}
                  options={perfilOptions}
                  placeholder="Não sei / tanto faz"
                />
              </div>
            </div>
            ...
            <div>
              <Label htmlFor="msg">
                {modo === "reescrever"
                  ? "Sua mensagem (vou deixar melhor)"
                  : "Mensagem da cliente"}
              </Label>
              <Textarea
                id="msg"
        ...
        <div className="space-y-4">
          ...
          {freeRemaining !== null && freeRemaining > 0 && !limitReached && (
            <div className="rounded-xl border border-gold-500/30 bg-gold-500/8 px-4 py-2.5 text-xs text-champagne-300">
              Plano grátis ·{" "}
              <strong className="text-gold-400">faltam {freeRemaining}</strong>{" "}
              {freeRemaining === 1 ? "resposta grátis" : "respostas grátis"}.
              Assine o Start pra liberar ilimitado.
            </div>
          )}
          ...
          {limitReached && (
            <div className="rounded-2xl border border-gold-500/40 bg-gold-500/10 p-6 text-center shadow-card">
              ...
              <p className="text-sm text-navy-100 mb-4 max-w-[340px] mx-auto leading-relaxed">
                Assine o{" "}
                <strong className="text-gold-400">Start (R$97/mês)</strong> e
                gere respostas ilimitadas, no tom da sua clínica.
              </p>
              <button
                type="button"
            ...
            <div className="rounded-2xl border border-navy-500 bg-navy-800 p-4 shadow-soft">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  {
                    label: "Intenção",
                    val: nlp.intent?.replace(/_/g, " ") ?? "—",
                  },
                  { label: "Sentimento", val: nlp.sentiment ?? "—" },
                ].map((item) => (
                  <div key={item.label} className="col-span-1">
                    <p className="text-[10px] uppercase text-navy-100 font-medium">
                      {item.label}
                    </p>
