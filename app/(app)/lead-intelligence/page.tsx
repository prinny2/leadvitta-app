  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <svg width="180" height="180" viewBox="0 0 180 180">
        <circle cx="90" cy="90" r={R} fill="none" stroke={tk.ringTrack} strokeWidth="12" />
        <circle
          cx="90" cy="90" r={R} fill="none"
          stroke={ringColor} strokeWidth="12" strokeLinecap="round"
          strokeDasharray={circ} strokeDashoffset={offset}
          transform="rotate(-90 90 90)"
          style={{ transition: "stroke-dashoffset 1.2s cubic-bezier(0.34,1.56,0.64,1)", filter: `drop-shadow(0 0 8px ${ringColor}80)` }}
        />
        <circle
          cx="90" cy="90" r={R} fill="none"
          stroke={ringColor} strokeWidth="2" strokeDasharray={circ} strokeDashoffset={offset}
          transform="rotate(-90 90 90)"
          style={{ opacity: 0.3, transition: "stroke-dashoffset 1.2s cubic-bezier(0.34,1.56,0.64,1)" }}
        />
        <text x="90" y="82" textAnchor="middle" fill={tk.textPrimary} fontSize="36" fontWeight="700" fontFamily="serif">{score}</text>
        <text x="90" y="104" textAnchor="middle" fill={ringColor} fontSize="12" fontWeight="600">SCORE</text>
      </svg>
      <span className="rounded-full px-4 py-1.5 text-sm font-bold"
        style={{ background: `${ringColor}18`, border: `1px solid ${ringColor}45`, color: ringColor }}>
        {tempLabel}
      </span>
    </div>
  );
...
// ── Insight card ──────────────────────────────────────────────────────────────
function InsightCard({ icon: Icon, label, text, color, tk }: {
  icon: React.ElementType; label: string; text: string; color: string;
  tk: ReturnType<typeof useTokens>;
}) {
  return (
    <div className="rounded-2xl p-4" style={{ background: `${color}0A`, border: `1px solid ${color}28` }}>
      <div className="flex items-center gap-2 mb-2">
        <div className="flex h-7 w-7 items-center justify-center rounded-lg" style={{ background: `${color}20` }}>
          <Icon size={14} style={{ color }} />
        </div>
        <p className="text-[10px] uppercase tracking-widest font-bold" style={{ color: `${color}BB` }}>{label}</p>
      </div>
      <p className="text-sm leading-relaxed" style={{ color: tk.textPrimary }}>{text}</p>
    </div>
  );
}
...
export default function LeadIntelligencePage() {
  ...
  function irParaGerador() {
    if (!mensagem) return;
    try { sessionStorage.setItem("re_prefill", JSON.stringify({ mensagem })); } catch { /**/ }
    router.push("/gerador");
  }
    ...
    <div className="space-y-5 animate-fade-in max-w-4xl">
      ...
      >
        <div
          ...
          style={{ background: "radial-gradient(ellipse at 90% 50%, rgba(201,160,96,0.07) 0%, transparent 60%)" }}
        />
        <div className="relative flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h1 className="font-serif text-2xl sm:text-3xl font-semibold" style={{ color: tk.textPrimary }}>
                Lead Intelligence
              </h1>
              <span
                className="rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider"
                style={{ background: "rgba(201,160,96,0.18)", border: "1px solid rgba(201,160,96,0.4)", color: "#C9A060" }}
              >
                PRO
              </span>
            </div>
      ...
      >
        <label className="block text-xs font-bold uppercase tracking-widest mb-3" style={{ color: tk.textMuted }}>
          Mensagem da Cliente
        </label>
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {EXEMPLOS.map((ex) => (
            <button
              ...
              style={{ background: tk.chipBg, border: `1px solid ${tk.chipBorder}`, color: tk.textSecondary }}
            >
        ...
        <div ref={resultRef} className="space-y-4 animate-fade-in">
          <div className="grid gap-4 sm:grid-cols-2">
                ...
                {(Object.entries(result.eixos) as [string, number][]).map(([key, val]) => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-2">
                      <span className="w-24 text-[11px] shrink-0" style={{ color: tk.textMuted }}>{labels[key]}</span>
                      <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: tk.chipBg }}>
                        <div
                      ...
                      <span className="w-8 text-[11px] text-right font-semibold shrink-0" style={{ color: tk.textPrimary }}>{val}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div
              ...
              style={{ background: tk.surface, border: `1px solid ${tk.border}` }}
            >
              <p className="text-[10px] uppercase tracking-widest font-bold mb-2" style={{ color: tk.textMuted }}>
                Radar de Perfil
              </p>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <InsightCard icon={Target}     label="Perfil do Lead"    text={result.perfil}    color="#C9A060" tk={tk} />
            <InsightCard icon={TrendingUp} label="Melhor Abordagem"  text={result.abordagem} color="#6B8CAE" tk={tk} />
            <InsightCard icon={Zap}        label="Gatilho de Venda"  text={result.gatilho}   color="#A0C9A0" tk={tk} />
          </div>
          ...
          >
            <div>
              <p className="font-semibold" style={{ color: tk.textPrimary }}>Pronta para responder?</p>
          ...
          {[
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                <span className="font-serif text-2xl font-bold" style={{ color: "rgba(201,160,96,0.35)" }}>{s.step}</span>
                <s.icon size={16} style={{ color: "#C9A060" }} />
              </div>
              <p className="font-semibold text-sm mb-1" style={{ color: tk.textPrimary }}>{s.title}</p>
              <p className="text-xs leading-relaxed" style={{ color: tk.textSecondary }}>{s.desc}</p>
            </div>
          ))}
        </div>
      )}
