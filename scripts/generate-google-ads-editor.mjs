...
const ga4Conversions = {
  propertyId: "G-223KR63TS8",
  conversionsToImport: [
    { name: "sign_up", category: "Sign-up", includeIn: "Conversions" },
    { name: "begin_checkout", category: "Begin checkout", includeIn: "Conversions" },
    { name: "purchase", category: "Purchase", includeIn: "Conversions", value: true },
    { name: "generate", category: "Generate response", includeIn: "All" },
  ],
  linkInstructions: "Link the GA4 property to this Google Ads account in Google Ads > Tools > Conversions > Google Analytics 4.",
};
...
function ga4Markdown() {
  const lines = [
    "# GA4 to Google Ads Conversions Setup",
    "",
    `Generated by \`npm run ads:generate\`. GA4 property: \`${ga4Conversions.propertyId}\``,
    "",
    ...
    `3. Link property \`${ga4Conversions.propertyId}\` (LeadBellus).`,
  ...
  ga4Conversions.conversionsToImport.forEach((c) => {
    lines.push(`- **${c.name}** — Category: ${c.category} — Include in: ${c.includeIn}${c.value ? " (use value from GA4)" : ""}`);
  });
...
async function main() {
  ...
  await writeFile(path.join(outDir, "07_ga4_conversions.md"), ga4Markdown(), "utf8");
  ...
  console.log(`GA4 conversions documented: ${ga4Conversions.conversionsToImport.length} (property ${ga4Conversions.propertyId})`);
}
