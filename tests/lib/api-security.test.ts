...
describe("rejectCrossOriginRequest", () => {
  ...
  it("permite origem igual à do próprio request", () => {
    expect(
      rejectCrossOriginRequest(req({ origin: "http://localhost:3000" })),
    ).toBeNull();
  });
  ...
  it("permite referer autorizado", () => {
    expect(
      rejectCrossOriginRequest(
        req({ referer: "http://localhost:3000/pagina" }),
      ),
    ).toBeNull();
  });
...
describe("enforceRateLimit", () => {
  ...
  it("separa contadores por IP", () => {
    ...
    expect(
      enforceRateLimit(req({ "x-forwarded-for": "3.3.3.3" }), opts),
    ).toBeNull();
    // IP diferente => contador independente, ainda permitido.
    expect(
      enforceRateLimit(req({ "x-forwarded-for": "4.4.4.4" }), opts),
    ).toBeNull();
  });
